...
(Codex, Anthropic Agent SDK, etc.) working in this repository.
...
- Skill structural validation (Gold Standard / CSO):
  ```bash
  python3 .Codex/skills/skill-creator/scripts/validate_skill.py skills/<skill>
  ```
...
5. **Re-run skill-validator** on all four office skills:
   ...
   for s in docx xlsx pptx pdf; do
       python3 .Codex/skills/skill-creator/scripts/validate_skill.py skills/$s
   done
   ```
...
or write to the plan file in `~/.Codex/plans/` if in plan-mode) and
...
- [`.Codex/skills/skill-validator/`](.Codex/skills/skill-validator/)
