...
| **R13** | `xlsx-8a-11` — Expose `--memory-mode {auto,streaming,full}` CLI flag so callers can explicitly opt into streaming for large workbooks without code change. Surfaced by `tracemalloc` + `resource.getrusage` measurement on `Моделирование.xlsx` 2026-05-13. **In-process measurement** (with tracemalloc instrumentation overhead): 1188 MB → 278 MB RSS (4.3×) at +35% wall-clock penalty. **Subprocess measurement post-implementation** (no instrumentation, cold-start): 1188 MB → **156 MB** RSS (**7.6×**) AND **12.6 s → 32.4 s** wall-clock (**2.6× speedup**) — the streaming mode is actually faster than full-load on this file because openpyxl's non-read-only path eagerly materialises every cell-object + style-ref at `load_workbook` time. Either way, 4-8× memory reduction available but was not exposed in CLI. | ✅ | (a) **Root cause**: `xlsx2csv2json.dispatch._dispatch_to_emit` hard-codes `read_only_mode = False if args.include_hyperlinks else None`; user can't pass `True` from CLI without editing code. (b) **Fix**: add `--memory-mode {auto,streaming,full}` argparse entry in [`xlsx2csv2json/cli.py`](../skills/xlsx/scripts/xlsx2csv2json/cli.py); default `auto` preserves current behaviour (size-threshold-driven). (c) **Translation**: `auto` → `read_only_mode=None` (auto); `streaming` → `read_only_mode=True` (force openpyxl streaming, merge-aware features no-op per R12 honest-scope); `full` → `read_only_mode=False` (force non-streaming, all merge features work, memory unbounded by file size). (d) **Conflict handling**: `--include-hyperlinks` + `--memory-mode streaming` is structurally impossible (ReadOnlyWorksheet doesn't expose `cell.hyperlink`); emit a stderr warning and override to `full` (mirrors existing `--include-hyperlinks` → `read_only_mode=False` behaviour). (e) **5 tests**: `test_R13_memory_mode_auto_preserves_default` (regression), `test_R13_memory_mode_streaming_forces_read_only` (lib param threaded through), `test_R13_memory_mode_full_forces_non_read_only` (size-threshold-override), `test_R13_streaming_with_hyperlinks_warns_and_overrides` (conflict resolution), `test_R13_memory_mode_streaming_modeling_xlsx_under_500MB_rss` (E2E `resource.getrusage` measurement on the trigger workbook; locks 4× win). |
