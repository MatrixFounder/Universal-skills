...
All mechanical gates are green (333 unit tests, E2E 157/157, `validate_skill.py` PASSED, A11
clean). Three adversarial cycles ran, the workflow's cap. Cycle 3 confirmed 5 defects and the
repairs for them were applied by the orchestrator **after** the last cycle, so they have had
**no review pass**. `vdd-enhanced` §4.5 makes that a WARNING rather than a PASS, and it is
recorded as one. 54 findings raised by the cycles remain unverified and are tracked in
...
| R10 | **Fenced code and inline code are inert.** No rule R3, R4, R8, or R9 may rewrite text inside a fenced block, an indented code block, or an inline code span. | YES | (a) fenced blocks (triple-backtick and triple-tilde, any info string) masked before transformation and restored after; an **indented** block is masked only when CommonMark would read it as code — after a blank line, and **not inside a list**. Obsidian indents nested bullets with a tab, so a mask without that precondition swallowed every nested list item and let `![[embed]]` and `[[wikilink]]` through untouched at exit 0 (cycle-1 finding L-1, the round's only CRITICAL). A masked block's effect on the walk is decided by **its own original indentation**, recovered from the mask store: an indented fence is list content and leaves the list open, a column-0 fence closes it. `keep()` swallows that indentation, so the sentinel alone cannot answer the question. Treating it as a flush line masked the next continuation as code (cycle-2 O-1); treating it as always neutral leaked the list state past the list's end, leaving a genuine top-level indented code block visible to every rewrite — `#include <stdio.h>` came back as `<stdio.h>` (cycle-3 C3-01). **Honest scope:** an indented code block *nested inside a list* is treated as list content and stays visible to the rewrites; (b) inline code spans masked likewise; (c) a `mermaid` block passes through byte-identical — regression against the existing mermaid path; (d) frontmatter detection anchors at offset 0 only, so a `---` rule mid-body is never eaten |
