...
### 1.3. Что такое text-humanizer (второй сквозной пример)

> **📦 Пример из практики — text-humanizer**
>
> `text-humanizer` — скилл из этого репозитория. Его задача: взять готовый текст и убрать из
> него следы того, что его писала нейросеть, — типичные слова вроде «delve» и «testament»,
> абзацы одинаковой длины, слишком гладкие переходы между мыслями.
>
> **Почему мы на него ссылаемся:** у `wiki-verify` показано, как выглядят зрелые
> eval'ы. У `text-humanizer` показано другое — **как их строят с нуля и на чём при
> этом ошибаются**. Почти все его примеры в гайде — это найденные дефекты: сначала
> в самом скилле, потом в собственном измерительном инструменте. Исходники и все
> кампании: `skills/text-humanizer/evals/`.
...
### 7.8. Вызов скилла проверяют по ответу, а не по логу

**Принцип.** Чтобы понять, вызвался ли скилл, обычно смотрят в лог: не встречается ли там
обращение к скиллу. Способ ненадёжный. Во-первых, он не отличает «скилл не выбрали» от «выбрали,
но не стали применять». Во-вторых, если в логе обращение по какой-то причине не нашлось, ответ
будет тот же самый — «не вызвался».

Надёжнее сделать так, чтобы вызов скилла было **видно по самому ответу**. Для этого скиллу дают
инструкцию, выполнение которой ни с чем не спутаешь: например, заменять одни слова на другие.
Если в ответе стоит `depot` там, где в исходном тексте было `warehouse`, — значит инструкция из
скилла до модели дошла и была выполнена. Проверяется обычным поиском по строке.

> **📦 text-humanizer.** Для проверки собрали скилл-двойник: то же описание, но тело — список из
> двенадцати замен (`warehouse` → `depot` и так далее). Дальше четыре набора прогонов:
>
> 1. **испытуемое описание** — то, что и хотим измерить;
> 2. **описание, написанное точно под запрос** — показывает, чего вообще можно добиться
>    описанием, если постараться;
> 3. **скилла нет вообще** — сколько замен модель сделает сама, без всякого скилла;
> 4. **запрос прямо называет скилл** — проверка, что стенд вообще работает.
>
> **Пока четвёртый набор не сработал, первые три читать нельзя.**

### 7.9. Результат «0 из 30» ничего не значит, пока стенд не показал ненулевого

**Принцип.** Триггерный eval часто заканчивается строкой вида «сработало 0 прогонов из 30».
У такого результата два прочтения: **скилл действительно не вызвался** или **стенд не умеет
увидеть, что он вызвался**. Само число их не различает — оно одинаковое в обоих случаях.

Поэтому в наборе всегда нужна руками устроенная проверка: прогон, который **обязан** сработать
(например, запрос прямо называет скилл). Пока она не дала ненулевого результата, все остальные
нули читать нельзя — они могут быть поломкой стенда.

> **📦 text-humanizer.** Три раза за вечер «0 из 30» оказалось поломкой стенда, а не находкой.
>
> **Первый.** Скилл-двойник назвали так же, как настоящий скилл. Настоящий его перекрыл, и до
> двойника запросы вообще не доходили — а на вид это был честный ноль.
>
> **Второй.** На дешёвой модели ноль дали все описания подряд, включая заведомо удачное. Значит
> эта модель просто не различает описания — а не «описания не важны».
>
> **Третий.** Прогон, упавший по таймауту, засчитался как «скилл не сработал». Считайте такие
> прогоны отдельно и печатайте их число рядом с результатом.

### 7.10. Мерить на той модели, которая исполняет скилл

**Принцип.** У моделей разная цена. Дешёвая (например, Haiku) заманчива для таких прогонов:
она отвечает быстро, и повторить сто раз почти ничего не стоит. Но **решение «взять скилл или
сделать самому» модели принимают по-разному**, и на дешёвой можно получить вывод, обратный
правильному.

Правило простое: дешёвой моделью **отлаживают сам стенд**, а результат читают на той модели,
которая будет работать со скиллом на самом деле.

> **📦 text-humanizer.** Одни и те же описания, одни и те же тексты. В таблице — доля прогонов,
> в которых скилл сработал:
>
> | Что стоит в описании | Haiku (дешёвая) | Sonnet (рабочая) |
> | :--- | ---: | ---: |
> | запрос прямо называет скилл | 1.00 | 0.93 |
> | описание, написанное специально под такой запрос | **0.00** | **0.90** |
> | описание, которое стоит в скилле сейчас | 0.00 | **0.10** |
>
> По колонке Haiku напрашивался вывод: «описание ни на что не влияет, все варианты дают ноль».
> Колонка Sonnet показывает, что это неправда — там разница между двумя описаниями девятикратная.
> А работает со скиллом именно Sonnet.

> **✅ Что запомнить из §7 (десять принципов разом):**
...
> 8. Вызов скилла проверяйте **по ответу**, а не по логу.
> 9. **«0 из 30» читайте последним** — сначала пусть сработает прогон, который обязан сработать.
> 10. Триггерные прогоны читайте **на рабочей модели**, а не на дешёвой.
...
- [ ] В наборе есть прогон, который **обязан сработать**, и он сработал — иначе остальные нули могут быть поломкой стенда.
- [ ] Триггерные прогоны читаются на той модели, которая будет работать со скиллом, а не на дешёвой.
- [ ] Нет проверок, которые **не пройдёт и правильный ответ** (например, требование дословной цитаты там, где скилл обязан переформулировать).
...
| Читать «0 из N», не проверив стенд | «не вызывается» может быть «не видим» | сначала прогон, который обязан сработать |
| Судить о вызове скилла по дешёвой модели | на ней ноль дают все описания подряд, и различить их нельзя | результат читать на рабочей модели |
| Упавший прогон засчитан как «не сработало» | поломка стенда читается как плохое описание | считать упавшие отдельно и печатать их число |
| Проверка, которую не пройдёт и правильный ответ | требовать дословную цитату в 20 слов там, где скилл обязан переформулировать | проверять имена, числа и идентификаторы; для формулировок перечислять допустимые варианты |
| Стенд читает то, что варьирует, с диска | в тот же файл пишет другая кампания | стенд сам записывает переменную и печатает её |
