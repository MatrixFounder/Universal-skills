...
|---|---|---|
| **docx** | Markdown ↔ Word, Obsidian→Word, math (OMML), HTML→docx, surgical edit, comments, tracked-changes, template fill, validate | [arch-011](architectures/architecture-011-docx-skill.md) · surgical-edit detail [arch-006](architectures/architecture-006-docx-replace.md) |
...
| [011](architectures/architecture-011-docx-skill.md) | **docx** skill · `docx-*` (TASK 006/008/019/030/031) | `md2docx.js` + `_math_lib.js`, `obsidian2md.js` + `_obsidian_lib.js`, `docx2md.js`, `html2docx.js`, `docx_{replace,add_comment,fill_template,accept_changes,merge}.py` | docx as OOXML **master**; full capability set + TASK 019 hardening + TASK 030 Obsidian input + TASK 031 math (OMML); cross-refs arch-006 & arch-010; §8 = TASK 030 Obsidian input, §9 = TASK 031 math support | 763 |
