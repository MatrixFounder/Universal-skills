...
| docx-2 | `docx_merge.py` ✅ DONE (VDD-hardened) | Склеивает N `.docx` в первый (base) через `office.unpack` + body-children-копирование с insert-before-sectPr; копирует недостающие `<w:style>` definitions (matched by `w:styleId`); опц. `--page-break-between` вставляет `<w:br w:type="page"/>` перед каждым добавляемым; `--no-merge-styles` отключает копирование стилей. Honest scope: numbering / footnotes / endnotes / headers / footers / images / comments **НЕ мерджатся** — warning эмиттится только когда extra-input реально содержит user content (boilerplate md2docx-fixtures игнорируются: numbering > 1 num, footnotes > 2 boilerplate, comments > 0). Same-path I/O (output совпадает с любым из inputs, включая symlink) → exit 6 `SelfOverwriteRefused`. E2E: 8 проверок (3-input merge + validate + ordering preserved + page-breaks count + single-input NotEnoughInputs envelope + cfb cross-3 rejection + VDD-B same-path + VDD-B symlink). | M | M | — | Аналог `pdf_merge`, но через body-tree concat вместо PDF-page concat. Numbering renumbering / bookmark-id remapping — сложно и редко нужно для «преамбула + статьи + приложения», поэтому отложено. |
