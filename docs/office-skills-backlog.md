...
| ID | Название | Что/Зачем | Effort | Value | Dep | Notes |
|---|---|---|---|---|---|---|
| cross-1 | `preview.py` универсальный ✅ DONE | Один CLI: на вход docx/pptx/xlsx/pdf — на выход PNG-grid (по странице/слайду/листу). Байт-идентичен в 4 скиллах. `.pdf` идёт напрямую через pdftoppm; `.docx/.xlsx/.pptx` (вкл. `.docm/.xlsm/.pptm`) через soffice → pdftoppm. E2E: 3 проверки в pdf. | M | M | pptx_thumbnails | Заменяет 4 разных способа «глянуть, как это выглядит». |
...
| cross-4 | `.docm`/`.xlsm`/`.pptm` (с макросами) ✅ DONE | Read-only поддержка через `office/_macros.py`: `is_macro_enabled_file()` детектирует по vbaProject.bin или по macroEnabled content-type; writer-скрипты (docx_fill_template, docx_accept_changes, xlsx_recalc, xlsx_add_chart, pptx_clean) предупреждают, когда выходное расширение теряет макросы; `office.unpack` печатает note на macro-enabled входе; `office.pack` предупреждает при pack `.docm`-tree → `.docx`. E2E: 3 проверки в docx + 2 в xlsx + 2 в pptx. | S | L | — | Корпоративные пользователи иногда дают такие файлы. |
| cross-5 | Унифицированный error-reporting ✅ DONE | `scripts/_errors.py` (4-skill копия): `add_json_errors_argument(parser)` + `report_error(msg, code, error_type, details, json_mode)`. JSON envelope: `{"error", "code", "type"?, "details"?}`. Подключён во все 13 Python CLI. E2E: 4 проверки (по одной на скилл). | M | L | — | Полезно для обёрток-агентов. |
