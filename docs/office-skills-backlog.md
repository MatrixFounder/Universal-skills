...
- **pdf-11 v1 (Chrome-headless engine via Playwright) ✅**: opt-in
  альтернативный движок для случаев, когда weasyprint объективно не
  справляется (Material 3 calc/var bugs, Framer infinite-loop, ELMA365
  inline.py assertion, JS-hydrated content, `<canvas>` charts).
  CLI: `--engine weasyprint|chrome` (default: weasyprint — backwards
  compatible). `chrome` — рендер через bundled headless Chromium
  (`page.pdf()`, A4/Letter/Legal margins 1cm, `print_background=True`),
  network blocked через Playwright route-handler (паритет с weasyprint
  `_offline_url_fetcher`). Lazy import Playwright — отсутствие пакета
  даёт fail-loud `ChromeEngineUnavailable` envelope с remediation.
  Soft-optional dep: отдельный `requirements-chrome.txt`,
  `bash install.sh --with-chrome` ставит Playwright + Chromium (~150 MB).
  Chrome path **не запускает** weasyprint preprocess pipeline (calc-strip,
  font-face-strip, NORMALIZE_CSS) — это weasyprint workarounds, Chrome
  их не нуждается; reader-mode и `--css EXTRA.css` остаются engine-
  agnostic. 6 unit-тестов pinning контракт (default=weasyprint,
  unknown engine raises, page-size mapping, preprocess skipped on chrome,
  ChromeEngineUnavailable message). E2E: gmail_example (NumberToken
  crash в weasyprint) → 56 KB PDF; elma365_activities (inline.py crash
  в weasyprint) → 644 KB / 7 страниц; ya_browser → 169 KB / 3 страницы.
  Honest scope (отложено в pdf-11a): `--engine auto` с structural
  pre-scan (calc-count, canvas-count, virtualizer-markers) и engine-
  decision cache в side-car file; cross-platform validation
  (Linux Alpine/RHEL, Docker `--no-sandbox`, Lambda chrome-min); chrome
