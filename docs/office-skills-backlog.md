...
| docx-11 | Математика (`$…$` / `$$…$$`) в `md2docx.js` | Формулы уезжают в документ литеральным текстом (`grep katex\|mathml md2docx.js` = 0). В скиле `pdf` решено пререндером KaTeX → MathML (`md2pdf.js`); для docx честных варианта два — OMML через `docx`-библиотеку или растр формулы в png. Вынесено из TASK 030 (спецификация §8 R-3) как самостоятельная задача. | L | M | — | Не делать внутри obsidian-задачи: утопит остальное. Обратить внимание на inline-vs-display и на нумерацию формул. |
