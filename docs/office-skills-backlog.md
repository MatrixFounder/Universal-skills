...
| docx-6 | `docx_replace.py` (точечный edit без шаблона) | Edit живого .docx без заранее размеченных `{{placeholder}}`: найти текстовый якорь и заменить / вставить параграф / удалить — **сохраняя run-level форматирование** (стили, нумерацию, inline-разметку). Сейчас агент идёт через `docx2md → правка → md2docx` round-trip и теряет форматирование. Архитектура: переиспользовать `office.unpack` + lxml + run-merge паттерн из `docx_add_comment.py` (anchor-finding с учётом split runs). Минимальный API: `--anchor TEXT --replace TEXT` (in-place text swap внутри run); `--anchor TEXT --insert-after PATH` (вставить параграф из MD-файла); `--anchor TEXT --delete-paragraph`. Anchor not found → exit 2 `AnchorNotFound` envelope. `--all` для multi-match. Cross-3/4/5/7 параллель. **Honest scope (v1)**: anchor должен помещаться в один `<w:t>` после run-merge (не пересекаем формат-границы) — документировано как и в docx-1. | M | H | — | Закрывает gap «edit existing docx без потерь форматирования». Паттерн уже отработан в docx-1 (`_wrap_anchors_in_paragraph` cursor-loop). Use-case: агент-редактор, который правит контракт по комментам не теряя нумерацию/стили. |
...
| xlsx-8 | `xlsx2csv.py` / `xlsx2json.py` (read-back) | Обратная конверсия .xlsx → CSV (per-sheet) или JSON (массив объектов / dict-per-sheet). Сейчас csv2xlsx односторонний, и чтобы агент прочитал пользовательский xlsx — нужно либо ручной unpack XML, либо запуск `xlsx_check_rules` с пустыми правилами. Это footgun для use-case «проанализируй мою таблицу». Реализация — тонкая обёртка над openpyxl (read-only mode для больших файлов): один CLI с `--format csv\|json`, опции `--sheet NAME\|all`, `--header-row N` (default 1), `--include-formulas` (по умолчанию вычисленные значения), `--include-hidden`, `--datetime-format ISO\|excel-serial`. JSON-форма: `[{header1: value1, ...}, ...]` per sheet, dict-of-arrays при `--sheet all`. Cross-3/4/5/7 параллель. Output на stdout если `--output` не задан. **Honest scope**: формулы по умолчанию резолвятся к cached value (требует прогонять xlsx_recalc заранее, иначе stale); rich-text/comments/styles теряются (text-only extraction); merged cells — значение в anchor cell, остальные `null`. | S | H | — | Симметричная пара к csv2xlsx / json2xlsx (xlsx-2). Bread-and-butter для агента-аналитика: «вот xlsx, разбери и посмотри». |
...
xlsx-4, pdf-3, pdf-4, pdf-7, cross-2) добавлены **3 новые задачи
агентского read/edit-loop'а** (docx-6, xlsx-8, pdf-12) — закрывают
структурные пробелы, обнаруженные при ревью backlog'а.

**P0 — universal hit-rate, прямой агентский запрос:**

- **xlsx-8** `xlsx2csv.py` / `xlsx2json.py` (S) — **closes critical
  gap**: csv2xlsx односторонний, агент сейчас не может прочитать
  пользовательский xlsx без ручного XML-unpack'а. Bread-and-butter
  для «проанализируй мою таблицу».
- **pdf-12** `pdf_extract_text.py` / `pdf_extract_tables.py` (M) —
  **closes critical gap**: для не-scanned PDF нет программного
  ...
  pdfplumber.
- **xlsx-2** `json2xlsx.py` (S) — натуральный output агента — JSON;
  самый частый «положи в Excel». Параллель к csv2xlsx.
- **xlsx-3** `md_tables2xlsx.py` (S) — агент постоянно работает с
  MD-таблицами (тех. документация, ответы LLM).
...
```
day 1:  xlsx-2 (S) → xlsx-3 (S) → xlsx-8 (S) → pdf-7 (S)   ← четыре S-победы
day 2:  pdf-12 (M) → pdf-4 (M)                              ← read-loop для PDF
day 3:  docx-6 (M) ИЛИ xlsx-4-аудит (M)                     ← edit-loop / integrity
```
