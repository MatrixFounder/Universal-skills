...
| docx-3 | `html2docx.js` ✅ DONE | CLI читает три формата браузерных сохранений и склеивает их в `.docx`: (a) plain `.html`/`.htm` (и Chrome "Save Page As, Webpage Complete" с сиблингом `<page>_files/` — относительные `<img src>` резолвятся от директории html), (b) Safari `.webarchive` (Apple binary plist через `bplist-parser`: `WebMainResource` → main HTML; `WebSubresources` image/* → tmp-dir; URL → local-path map с двумя ключами — full URL и path-only `/download/...?v=1` — потому что HTML в webarchive ссылается обычно по path-only), (c) Chrome `.mhtml`/`.mht` (RFC 822/2557 multipart/related: парсер на чистом Buffer + индекс-поиск boundary, поддержка LF и CRLF разделителей; декодинг quoted-printable и base64 для частей). Format-detection по расширению + magic-byte fallback (`bplist00`). DOM walker через cheerio: `<h1-h6>/<p>/<ul>/<ol>/<li>` (3 уровня), таблицы (включая авто-detection header row через `<thead>` или цельный `<th>`-ряд), `<blockquote>/<pre>/<code>/<hr>/<a href>/<img>/<br>` + inline `<strong>/<b>/<em>/<i>/<u>/<s>/<strike>/<del>/<code>`. Image-type detection с magic-byte fallback (Confluence-style assets без расширения: `atl.site.logo`, `global.logo`). Strip `<script>/<style>/<head>/<form>` и Confluence `<ac:*>` namespace (одно warning). Cross-7 H1 same-path guard (exit 6 + `SelfOverwriteRefused` envelope). Cross-5 inline `--json-errors` envelope (`{v:1, error, code, type, details?}`). Modular structure: `html2docx.js` (CLI, 225 LOC) + `_html2docx_archive.js` (extractors, 279 LOC) + `_html2docx_walker.js` (DOM walker, 435 LOC). E2E: 6 проверок (round-trip heading + table cell + same-path guard + JSON envelope + validate). Verified end-to-end на 4 реальных Confluence-выгрузках (Chrome HTML+_files/, .webarchive 33 MB, .mhtml 2.7 MB, Chrome HTML-only) — все генерируют валидный .docx через `office.validate`. | L | L | — | Прямой обход DOM (cheerio + docx-js), не через mammoth-обратный путь — у mammoth нет HTML→docx writer'а. Honest scope: `style=""` / CSS classes игнорируются, `rowspan`/`colspan` не воспроизводятся, remote `<img https://...>` пропускаются с alt-fallback. |
...
| pdf-5 | `html2pdf.py` ✅ DONE | Универсальный HTML→PDF конвертер через weasyprint — для BI-дашбордов, Confluence-страниц, сохранённых веб-страниц. Поддерживает три input-формата: `.html`/`.htm` (plain), `.mhtml`/`.mht` (MIME-архив, browser «Save as → Single File»), `.webarchive` (Apple Safari). Для MHTML/WebArchive sub-resources (изображения, CSS, шрифты) извлекаются в tempdir, URL переписываются на локальные пути. Preprocessing-pipeline: `_fix_light_dark()` — O(n) depth-counter замена `light-dark(X, Y)` → `X` (CSS Color Level 5 не поддерживается weasyprint, иначе SVG-диаграммы рендерятся чёрными); `_NORMALIZE_CSS` — инжект в `<head>` перед рендером: сбрасывает `body{overflow:clip; height:100%}` (SPA-страницы давали 1 страницу), fix `.drawio-macro svg` (overflow/min-width clipping), убирает fixed/sticky nav bars. Реализация: импортит `DEFAULT_CSS` и `PAGE_SIZES` из `md2pdf.py`. Опции: `--page-size letter|a4|legal`, `--css EXTRA.css`, `--base-url DIR`, `--no-default-css`. Cross-7 H1 same-path guard (exit 6 `SelfOverwriteRefused`). Cross-5 `--json-errors` envelope. E2E: 9 проверок (basic, `--no-default-css`, `--css`, missing input, same-path, `.mhtml` round-trip, `.webarchive` round-trip, unsupported ext → UnsupportedFormat, missing `--css` file → FileNotFound). Visual golden `html-basic.png`. | S | L | — | weasyprint, plistlib (stdlib), email (stdlib). Preprocessing решает реальные Confluence/Habr кейсы: drawio-диаграммы рендерятся цветными; статья Habr 1 стр → 3 стр. |
