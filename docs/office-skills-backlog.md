...
| docx-6.5 | Image relocator для `--insert-after` ✅ DONE 2026-05-12 (Task 008 atomic chain 8 sub-tasks + /vdd-multi hardening) | **v2 shipped:** новый docx-only sibling `_relocator.py` (839 LOC, no cross-skill replication) — F10 `_copy_extra_media` (фиксированный `insert_` префикс + collision-safe counter + symlink-reject + `_assert_safe_target` на каждый `src.name`), F11 `_max_existing_rid` (rId regex scan), F12 `_merge_relationships` (mergeable-type filter + rId offset + `_assert_safe_target` path-traversal guard CWE-22), F13 `_copy_nonmedia_parts` (chart/OLE/SmartArt verbatim default + collision rename + sibling `_rels/<basename>.rels` verbatim copy per D7 + symlink-reject), R4 `_remap_rids_in_clones`, R5 `_merge_content_types_defaults`. F16 `_assert_safe_target` с URL-decode double-pass (`urllib.parse.unquote` ловит `%2e%2e/`, `%2E%2E/`, `%2f` обход; обе формы прогоняются через 4 reject branches: `absolute_or_empty`/`drive_letter`/`parent_segment`/`outside_base`). R10.b WARNING DELETED; `_extract_insert_paragraphs` signature `(insert_tree_root, base_tree_root) -> tuple[clones, RelocationReport]`. Q-A2 stderr `[relocated K media, A abstractNum, X numId]` annotation when ≥ 1 asset relocated. G2 E2E `T-docx-insert-after-image-relocated` GREEN. G11 path-traversal E2E (`T-docx-insert-after-path-traversal`) exit 1 `Md2DocxOutputInvalid`. Honest scope: "supports all md2docx-produced relationship targets" (images, charts, OLE, SmartArt). | M | M | docx-6 | Pattern source `docx_merge.py:109-544` re-used BY COPY per Decision D3 (AST-walk regression-lock). Final metrics: **166 unit / 0 failures / 9 skipped, 151 E2E / 0 failed, POST_VALIDATE=1 hermetic 151/0, 12 diff -q silent, validate_skill ✅**. |
| docx-6.6 | Numbering definitions relocation для `--insert-after` ✅ DONE 2026-05-12 (Task 008 atomic chain + /vdd-multi hardening) | **v2 shipped:** F14 `_merge_numbering` в `_relocator.py` (port of `docx_merge.py:332-466`) — `<w:abstractNum>` + `<w:num>` copy с anum/num offset (max+1), ECMA-376 §17.9.20 abstractNum-before-num element ordering preserved (regression-lock `test_ecma_376_17_9_20_abstractnum_before_num_preserved`), включая **cleanup-only base case** (vdd-multi C-2: когда base has `<w:numIdMacAtCleanup>` но 0 `<w:num>`, abstractNums вставляются ДО cleanup); F15 `_remap_numid_in_clones` rewrites `<w:numId w:val>`; `_ensure_numbering_part` adds `[Content_Types].xml` Override + word/_rels Relationship. Install-verbatim branch when base has no `numbering.xml`. **Dangling-ref protection** (vdd-multi H-4: `_is_int` + `skipped_insert_anum_ids` + `insert_anum_ids_valid` — `<w:num>` whose `<w:abstractNumId w:val>` references skipped/missing insert def is dropped). **Size cap 8 MiB** на insert numbering.xml (vdd-multi H2 — DoS-resistance). R10.e WARNING DELETED. G3 E2E `T-docx-insert-after-numbering-relocated` (verifies offset-shift, base survival, ECMA-376 §17.9.20 ordering end-to-end) + G4 integration `T-docx-insert-after-image-and-numbering` GREEN. Q-A3 idempotency `test_relocator_idempotent_on_same_inputs` LIVE (weaker invariant: schema validity after two passes). | M | L | docx-6, docx-6.5 (shared rel-relocation infra) | Use-case: вставка numbered/bulleted lists из MD в любой docx без потери форматирования. Sibling rel-files copied verbatim (D7 — no recursive remap in v2). |
...
| docx-9 | venv self-bootstrap + A4 page-size + install verify ✅ DONE 2026-06-05 (TASK 019, full vdd pipeline) | Доработка по [`docs/docx-skill-improvement-spec.md`](docx-skill-improvement-spec.md) (реальный кейс md+Mermaid→A4). **(A, P0 bug)** Python-CLI падали `ModuleNotFoundError` когда системный `python3` ≠ `scripts/.venv` (pyenv/conda). Фикс: stdlib-only `scripts/_venv_bootstrap.py::reexec_into_venv()` re-exec'ит процесс в `.venv` (`os.execv`). **Детекция через `sys.prefix`, НЕ `realpath(sys.executable)`** — venv-symlink на pyenv указывает на тот же бинарь, realpath совпадает, спец-псевдокод не срабатывал бы (выявлено эмпирически). venv-absent → `run: bash install.sh`, не traceback. Прелюдия в 10 CLI-entrypoints (исключая 4 import-only helper'а — у них нет `__main__`, наследуют venv). **(B, P1 feature)** `md2docx.js`: `--page-size A4\|Letter` (default Letter, back-compat), `--landscape`, `--margins T,R,B,L` (dxa/`mm`); `contentWidthDxa`+image/Mermaid caps выводятся из геометрии (px=dxa/15), не из Letter-констант → таблицы не вылезают за A4 (9026 dxa); landscape через portrait-dims+`orientation:LANDSCAPE`. **(C, P2)** `install.sh` + dep-import-verify (ловит молчаливый wheel-fail на 3.14) + smoke-test bare `python3` (доказывает фикс A). Репликация (CLAUDE.md §2): `_venv_bootstrap.py`+`preview.py`→4-skill, `office/*`+`office_passwd.py`→3-skill; diff -q silent; validate_skill ×4 PASS; docx/xlsx/pptx/pdf suites green. Dogfood на tmp7: A4 одной командой, golden-parity (pgSz+5 tbl+4 media), 3 Mermaid рендерятся. Тесты: `test_venv_bootstrap.py` (8) + `test_md2docx_pagesize.py` (11). | M | H | — | Spec'овский realpath-guard был бы сломан на pyenv — починено через `sys.prefix`. Non-docx per-skill entrypoint wiring отложено (helper уже реплицирован — fast follow-up). |
| docx-10 | Поддержка Obsidian-заметок (`obsidian2md.js` + `md2docx.js --obsidian`) 🚧 IN PROGRESS (TASK 030) | Вольтовая заметка проходит через `md2docx.js` с exit 0 и молчаливой потерей: фронтматтер срезается (`md2docx.js:57`), `![[embed]]` и `[[wikilink]]` не распознаются marked (text-токены), пробел в пути к картинке даёт text-токен вместо image. Отдельный конвертер Obsidian-Markdown → CommonMark + тонкий флаг-обёртка. Постановка: [`docs/docx-skill-obsidian-support-spec.md`](docx-skill-obsidian-support-spec.md), TASK: [`docs/TASK.md`](TASK.md) (archived to `docs/tasks/task-030-docx-obsidian-support.md` on closeout). | L | L | — | Фронтматтер рендерится **таблицей** (требование пользователя), через существующий `table`-токен `md2docx.js` — те же бордеры и шапка `D5E8F0`, что у остальных таблиц. |
| docx-11 | Математика (`$…$` / `$$…$$`) в `md2docx.js` ✅ DONE 2026-08-14 (TASK 031, standard pipeline, не VDD) | Формулы больше не уезжают литеральным текстом. Пайплайн: пре-лексерная замена `$…$`/`$$…$$` на PUA-сентинелы в `_math_lib.js` (до `marked.lexer()` — иначе одиночные `_` внутри формул участвуют в CommonMark emphasis-pairing) → KaTeX (`output:"mathml"`, `trust:false`) → `mathml2omml` → OOXML `<m:oMath>` через `docx`-библиотеку (реализован именно этот из двух честных вариантов, растр — отклонён после feasibility-спайка). Разбиение на sentinel-куски применяется во ВСЕХ run-контекстах `parseInlineText` (text/strong/em/link, включая `<br>`-подразбиение и remote-image fallback), не только в plain-тексте — иначе формула внутри `**bold**` не рендерится, а протекает как сырые PUA-байты. Display-формула, единственная в параграфе/ячейке таблицы, получает отдельный центрированный параграф. Найдено при ручной проверке A9 на реальном 35-страничном документе (328 формул): Pandoc-таблица `|  | $$…$$ |  | (N) |` при равном делении на 4 колонки обрезала формулу до 1/4 ширины страницы — исправлено асимметричными колонками (3%/80%/3%/14%). Флаги `--no-math`/`--strict-math` (по аналогии с `pdf`). Честный скоуп: без автонумерации формул Word (`m:oMathPara`/`SEQ`) — номера документа уже вне `$$…$$` и не трогаются; формула в `**bold**` рендерится, но без жирного начертания (OOXML math run не наследует `w:rPr` параграфа); формула в alt-тексте локального изображения — только литеральный текст (`ImageRun.altText` — строка, не run). 26 unit/E2E-тестов (`test_md2docx_math.py`), `examples/fixture-math.md`. | L | M | — | Не делали внутри obsidian-задачи — сделано отдельной TASK 031. inline-vs-display учтено (R4d); нумерация формул сознательно вне скоупа (см. Rejected/Deferred в TASK 031, `docs/tasks/task-031-docx-math-support.md`). |
...
| pptx-6 | `pptx2md.py` — pptx → Markdown ✅ DONE (TASK 020) | Read-back конвертер (аналог `xlsx2md`): один `## Slide N` на слайд (title→`### `, body→nested bullets, tables→GFM, images→sidecar `media/`+links с sha1-dedup, notes; charts/EMF→`[kind]` placeholders; SmartArt silently skipped — v1). Пакет `scripts/pptx2md/` (cli/extract/images/ocr/emit/model/exceptions) + shim, pptx-specific (НЕ реплицируется; `_errors.py`+`office/_encryption.py` импортируются read-only). **Opt-in `--ocr`**: per-image tesseract (`eng+rus`, прямой subprocess — НЕ ocrmypdf/ghostscript/pytesseract, AGPL-clean). VDD: 6 atomic beads, adversarial roast каждого (логика+безопасность), нашли+починили реальные баги (shape_type/group crash, DecompressionBomb DoS); 59 unit + e2e. Dogfood: 6 декингов tmp8 (slodes-3 82сл/16MB/136img → 0.23s; slides-5 image-only → --ocr вернул eng+rus текст). | M | H | — | pptx-аналог xlsx-9. Уточнения post-MVP: WMF/EMF теперь рендерятся в inline PNG (D-11); opt-in `--ocr-denoise` (pptx-7). |
| pptx-7 | `--ocr-denoise` (OCR noise reduction) ✅ DONE (TASK 021) | Opt-in (OFF по умолчанию — дефолтный вывод byte-identical) шумоподавление для `--ocr`: (R1) **size-gate** `--ocr-min-px` (default 48) — пропускает OCR на мелких декоративных иконках (vector-PNG исключены); (R2) **confidence-gate** `--ocr-min-confidence` (default 50) — tesseract в `tsv`-режиме, `ocr._filter_tsv` держит слова с `conf>=C`, **дропает картинку если <2 уверенных слов** (дискриминатор — КОЛИЧЕСТВО уверенных слов, НЕ среднее: mean-гейт ошибочно убивал плотные реальные скрины с UI-мусором; калибровка зафиксирована dogfood'ом) + чистит low-conf garble внутри оставленных блоков; (R3) **dedup** в `emit.render_deck` — идентичный OCR-блок (лого на многих слайдах) показывается один раз, ссылка на картинку остаётся. Всё в pptx-specific `ocr.py`/`emit.py`/`cli.py` (replication-границу не трогает). 18 unit (`test_ocr_denoise.py`) + adversarial logic-review (нашли+починили MED vector-size-gate exemption + LOW comma-locale conf). Dogfood tmp8 `--ocr --ocr-denoise`: slides-2 6→0, slides-6 12→8, slides-3 103→95, slides-5 3→3 (контент сохранён, удалён только шум — ground-truth проверка: каждый удалённый блок и в plain-режиме пустой/тривиальный). | S | M | pptx-6 | Найдено в эксплуатации (OCR-аудит корпуса tmp8). pdf-OCR (`pdf_ocr.py`/ocrmypdf) — другой движок (whole-page, output у ocrmypdf), эти фильтры не переносятся; его шумовой рычаг — встроенные cleanup-флаги ocrmypdf (`--clean`/`--deskew`/`--rotate-pages`/`--threshold`), **отдельная будущая задача**. |
...
| pdf-4 | `pdf_ocr.py` ✅ **DONE (TASK 018, real-engine verified 2026-06-03) — all 6 beads 018-01..06** | OCR scanned PDF через `tesseract` или `ocrmypdf`. Нужно для legacy-документов. **Status:** all beads merged incl. 018-04 (`--password` decrypt-to-temp via pikepdf) + 018-05 (`--deskew`/`--rotate-pages`/`--clean` with `osd`/`unpaper` prereq probes). Engine installed during dev (ocrmypdf 17.5.0 + tesseract eng/rus/osd + gs 10.07.1); **a real API bug was caught+fixed** (`ocrmypdf.ocr` takes input/output positionally, not as kwargs). Verified-for-real here: engine/lang envelopes, runner exception-mapping+atomicity, pikepdf decrypt (0600 / wrong+no-password / scratch-shredding), knob prereq refusals (osd/unpaper), and the real-ocrmypdf API call (`--skip-text` on a digital PDF). **✅ Verified (PR-1 satisfied, 2026-06-03):** the scan→OCR→`pdf_extract` composition (`TC-E2E-03`) was run on an engine-equipped host — `bash skills/pdf/scripts/tests/test_e2e.sh` → **82 passed, 0 failed** (the `pdf_ocr` 35-case suite + the real composition round-trip both green). On a sandboxed host the composition soft-skips (the spawned `tesseract` can't read ocrmypdf's nested temp dir) — a host limitation, not a code/engine issue. Two LOW items deferred to `docs/KNOWN_ISSUES.md` (PDF4-L1 sidecar non-atomic, PDF4-L2 list-langs rc). **Locked design (TASK 018):** обёртка над `ocrmypdf` → **searchable PDF** (raster + невидимый text-layer), default `--lang eng+rus`, soft-optional `--with-ocr` (lazy-import, `OcrEngineUnavailable`), default `--skip-text`; композиция `pdf_extract` exit 10 → `pdf_ocr` → `pdf_extract` digital re-read. Все hard-failure → exit 1 + envelope `type` (D-A1; без новых кодов, `10` остаётся за `pdf_extract`). MVP = beads 018-01/02/03/06; R5 password (018-04) + R9 image-prep (018-05) — post-MVP. Артефакты: `docs/TASK.md` / `ARCHITECTURE.md` / `PLAN.md` + 6 task-файлов `docs/tasks/task-018-01..06-*.md`; обзоры `docs/reviews/{task,architecture,plan}-018-review.md`. | M | M | — | Системная зависимость: tesseract (+eng,rus traineddata) + ghostscript (checked, not installed). `pdf_ocr.py` импортирует `_errors.py` read-only — без cross-skill репликации. |
...
| pdf-7 | TOC bookmarks (PDF outline) ✅ DONE | Проверено: weasyprint отдаёт PDF outline из `<h1-h6>` из коробки (UA-stylesheet `bookmark-level`) — `md2pdf` + `html2pdf` (движок по умолчанию), без CSS-флага. Движок `--engine chrome` получил паритет через `page.pdf(outline=True, tagged=True)` — Chromium строит outline из tagged-PDF structure tree, поэтому `tagged=True` обязателен, и chrome-PDF становится tagged-PDF. Регресс-тесты в `test_e2e.sh` фиксируют все три пути; `tests/_outline_probe.py` читает outline обратно через pypdf. Floor Playwright поднят до `>=1.42`. TASK 014 (2026-05-22). | S | M | — | weasyprint outline был out-of-box (backlog-заметка «только in-page links» устарела); chrome-движок получил паритет в TASK 014. |
...
| pdf-12 | `pdf_extract.py` + `references/pdf-to-markdown.md` ✅ **DONE (TASK 013, VDD develop-all 2026-05-22)** | Закрывает частый запрос «сконвертируй PDF в md» — снижает разнобой и закрывает тихий сбой на сканах. **Scope уточнён** относительно исходной формулировки (две CLI `pdf_extract_text.py` / `pdf_extract_tables.py` c `--format markdown`): полноценный «магический конвертер» НЕ делается — PDF это позиционированные глифы без семантической модели, композиция Markdown (заголовки, склейка таблиц между страницами, порядок чтения, описание схем) остаётся LLM-суждением. Поставка: **(1)** reference `references/pdf-to-markdown.md` — дерево решений (цифровой PDF → pdfplumber; скан/сложный layout → Read-инструмент / OCR), рецепт (per-page `extract_text()`+`extract_tables()` → промежуточный дамп → композиция агентом), типовые грабли (многоколоночность, безлинейные таблицы, разрыв таблицы между страницами, страницы-картинки, выносы заголовков). **(2)** helper `pdf_extract.py` — НЕ конвертер, а структурированный дамп: per-page text+tables в JSON (`{page_count, doc_scanned, scanned_pages, pages:[{n,text,tables,char_count,has_images,scanned}]}`). Ключевая часть — **детекция сканов**: страница ~0 символов + картинка → `scanned`; весь документ скан → громкий сигнал (exit `10` `DocumentScanned` + stderr с указанием на OCR / Read-инструмент), не молчаливая пустота. Контракт: argparse, `--json-errors` envelope (v=1), exit-коды 0/1/2/10, идемпотентность; `--layout`, `--password`. e2e + фикстуры (цифровой / скан-подобный / зашифрованный). pdfplumber (уже в `requirements.txt`); импортирует `_errors.py` read-only — без cross-skill репликации. | M | H | — | Артефакты TASK 013: `docs/TASK.md` / `ARCHITECTURE.md` / `PLAN.md` + 6 task-файлов `docs/tasks/task-013-01..06-*.md`, обзоры в `docs/reviews/{task,architecture,plan}-013-review.md`. Имя `pdf_extract.py` (не `pdf2md.py`) намеренно — имя не обещает готовый MD. OCR не бандлится — только детектируется. Композиция с pdf-4: `pdf_extract.py` → exit 10 `DocumentScanned` → запустить OCR → повтор. **Validation:** `pdf_extract` unit+E2E suite 34/34 green; `test_e2e.sh` 67/0 (incl. the 3-check `pdf_extract` block); `validate_skill.py skills/pdf` exit 0; cross-skill `diff -q` (`_errors.py` / `preview.py`) silent. Deviation D-01: fixtures build-at-runtime (skill `.gitignore` ignores `*.pdf`). Commit pending (no auto-commit per `/vdd-develop-all`). |

---

### html (formerly html2md) (новый скилл — Web/HTML → Markdown для Obsidian и агентных workflow)
...
adversarial-verified)** — см. правило в [`CLAUDE.md` §2 «html»](../CLAUDE.md)
...
(TASK 022) + `docs/ARCHITECTURE.md` §html + `docs/PLAN.md` (на фазе
планирования).
...
(`HTML_READER_URL`/`_PROVIDERS` → self-hosted Jina / любой reader; jina — дефолт).
...
через FETCH-лестницу). Всё в html-**owned** `acquire.py`/`cli.py`/`model.py`/`emit.py` —
...
(никакого автологина/2FA). `html login URL --save-state s.json` (headful) → headless
...
html-owned (`acquire.py`/`cli.py`/`_chrome_auth.py`/`_cookies.py`) — gated-мастера не тронуты,
...
| d-4 | Живой корпус не несёт референтов — `path:line` протухают молча | `agentic-development` v3.27.0/v3.28.0 добавил **референт**: ссылка `path:line` несёт рядом код-спан с именем символа или дословной подстрокой цитируемой строки, и резолвер её проверяет. Механизм **уже доступен здесь** симлинком — `python3 .agent/skills/documentation-standards/scripts/check_positional_refs.py --all docs/ARCHITECTURE.md docs/architectures`. Замер: `0 with a referent, 17 without (not examined), 36 unresolvable` — 53 координаты живого корпуса, ни одна не проверяема, 36 не резолвятся (голое базовое имя вместо пути от корня). Порядок: сначала аудит дрейфа, потом простановка — заякорить раньше значит припечатать те координаты, что уже указывают не туда. Готово, когда прогон даёт 0 unresolvable и ноль `REFERENT_ABSENT`/`REFERENT_AMBIGUOUS`. | M | M | — | **Ничего не требует и ничего не сломало:** координата без референта — *не проверена*, а не дефект, гейт зелёный. Четыре ревью-чек-листа (симлинками) уже требуют прогона и цитирования строки охвата и запрещают ревьюеру требовать референты там, где их нет. В соседнем проекте замерено: 26 из 142 координат указывали не туда, и резолвер не сообщал ни об одной. |
...
<!-- Якорь ниже — для `run-feedback file --as work-item`: стабильная точка
     вставки, не зависящая от номера/названия секции. Новые буллеты вставляются
     сразу ПОД строкой якоря (новые сверху). Не удалять и не перемещать. -->
<!-- feedback:discovered-issues -->
- **html: все fetch-движки падают на vitalik.eth.limo при живом сайте (2026-07-13)** — `html2md.py` дважды (2026-07, разные сессии) упал на `https://vitalik.eth.limo/general/2026/06/29/obfuscation1.html` с «all fetch engines failed (tried lite, chrome, remote)», при том что сайт жив: `curl -sI` → HTTP/2 200 (проверено 2026-07-13). Похоже на TLS/HTTP2/UA-особенность eth.limo-шлюза, которую не переживает ни один из трёх движков. Диагностировать `acquire.py` на этом URL (сравнить заголовки curl vs httpx/playwright), при подтверждении — поправить движок или задокументировать обход (`curl … | html2md --stdin`). Effort: S · Value: M
...
- **LibreOffice 26.2: CLI `macro:///` dispatch недетерминирован
  (2026-07-02)** — на 26.2 (i) читается только ПЕРВЫЙ
  `-env:UserInstallation=` в командной строке, (ii) на холодном
  профиле `macro:///`-аргумент молча теряется при first-run
  инициализации (soffice всё равно выходит с кодом 0); прогрев через
  `--terminate_after_init` / повторные запуски не дал детерминизма.
  `xlsx_recalc.py` переведён на `--convert-to` + seed
  `OOXMLRecalcMode=0` (профиль сидируется через новый
  `_soffice.run(profile_seed=…)`) и жёсткую пост-верификацию.
  `docx_accept_changes.py` остаётся на macro:/// (accept — операция
  над document model, конвертацией не выражается), но теперь сидирует
  макрос в ПРАВИЛЬНЫЙ профиль + `MacroSecurityLevel=0` и ГРОМКО падает
  (exit 1, bogus-output удаляется), если ревизии не приняты.
  **TODO**: мигрировать `docx_accept_changes.py` на UNO-мост —
  `soffice --accept=pipe,…;urp` + клиент с uno-биндингами
  (connection-based, без гонки CLI-диспатча).
  **⚠️/✅ Проверено 2026-07-02 (macOS 26.5):** bundled
  `LibreOfficePython.framework/...//MacOS/LibreOfficePython` НЕЛЬЗЯ
  запускать напрямую — macOS убивает процесс SIGKILL «Code Signature
  Invalid / Launch Constraint Violation» (CODESIGNING code 4).
  **НО путь жизнеспособен**: в тот же день eval-агент (docx evals v1,
  кейс D-01) довёл цепочку до рабочего состояния — копия бинаря
  bundled-python ВНЕ бандла + `codesign --force -s -` (ad-hoc
  переподпись снимает launch-constraint), затем
  `soffice --headless --accept=pipe,…;urp` и диспатч
  `.uno:AcceptAllTrackedChanges` через UNO — настоящий accept, гейт
  `verify_no_tracked_changes` чист, пакет пересохранён движком
  (10 частей). Дизайн миграции: переподписанную копию класть в
  кэш-директорию скилла, деградировать в текущий громкий отказ, если
  codesign недоступен. Артефакты прогона: skills/docx/evals/reports/.
- **xlsx с merged cells** — поведение `csv2xlsx` при импорте обратно
  не определено; вероятно теряет merge.
- **xlsx-8 §11.7 production-readiness gaps (R21-R29 polish chain
  vdd-multi-3 review, 2026-05-13)** — три критика прошли по всему
  changeset, нашли проблемы:
  - **HIGH (3-критика convergence)**: R23 `_whole_sheet_region`
    in-loop cap fire был silent → silent data truncation на
    hostile workbook (sparse data at row ≥ 1M). ✅ FIXED: emit
    `UserWarning` → `_emit_warnings_to_stderr` surfaces к юзеру.
  - **HIGH-Sec**: R21 `_is_reserved_name` без length-cap на input
    → ReDoS-via-length (50K names × 100KB each → 15 GB regex work).
    ✅ FIXED: `len(stripped) > 255` early-return (OOXML §18.2.6 max
    defined-name length).
  - **MED-Logic**: `max(ws.max_row, 1)` → TypeError если openpyxl
    в read_only mode вернёт None для отсутствующего `<dimension>`.
    ✅ FIXED: `max(ws.max_row or 0, 1)`.
  - **MED-Sec**: ZWSP-bypass на `_is_reserved_name` (`_xlnm​.Print_Area`
    проскальзывал). ✅ FIXED: strip zero-width chars (U+200B/200C/
    200D/FEFF) перед match.
  - **LOW-Logic**: stdout `--encoding utf-8-sig` silent
    (emit_csv намеренно drops BOM в pipe). ✅ FIXED: stderr warning
    при CSV-stdout + utf-8-sig.
  - **LOW-Data**: `_reserved_names.json` legacy bare-form pattern
    не покрывал `Disable_Reset`/`Data_Form`. ✅ FIXED.
  - Test additions: `test_in_loop_cap_emits_warning`,
    `test_whole_sheet_region_handles_none_max_row`,
    `test_long_input_not_matched_no_redos_surface` (с perf-budget
    < 50 ms на 1 MB input), `test_zero_width_chars_stripped_before_match`,
...
- **JSON keys-bloat на layout-heavy отчётах** ✅ FIXED 2026-05-13
  ...
  + contract + project + period + separator + real headers row 7»,
  но `flatten_headers` производил ` › `-concatenated ключи длиной
  100+ символов с metadata-префиксами, маскирующими реальные
  column-names. Real headers были в ключах **в конце**, но пользователь
  видел только префикс. Не `--skip-rows` (костыль, требует ручного
  счёта), а **`--header-rows leaf`**: auto-detect band + rsplit
  каждого ключа на последний ` › `-сегмент. Empty levels уже
  пропускаются в flatten join → автоматический fallback на
  next-deepest non-empty уровень. Library `xlsx_read` без изменений
  (она знает только `int | "auto"`); `leaf` живёт в shim's dispatch.
  Recipe для layout-heavy отчётов:
  `xlsx2json.py FILE OUT --tables auto --header-rows leaf --drop-empty-rows`.
  Размер masterdata.json 126 KB → 75 KB (40% меньше). Тесты:
  `test_header_rows_leaf_keeps_only_deepest_level_per_column` +
  `test_header_rows_auto_still_emits_multi_level_concat` (regression
  guard на старое R7-поведение).
- **JSON dict silent data loss при duplicate headers**
  ✅ FIXED 2026-05-12 (TASK 010 §11.7 R27): `_rows_to_string_style`
  в `emit_json.py` делал наивный `d[header] = value` без
  дедупликации — на layout-heavy отчётах с wide title-merge
  (`A1:F1` → 6 столбцов с одинаковым ключом) поздние записи
  молча перезаписывали ранние. **5 из 7 значений per row молча
  терялись на masterdata Timesheet** (только row 9 = «дата»
  столбец и «описание» столбец выживали). Fix: новый helper
  `_disambiguate_duplicate_headers(headers)` приписывает `__2`,
  `__3`, ... повторам. Зеркалит M2 vdd-multi precedent
  (`emit_csv._emit_multi_region` для colliding file paths). CSV
  emit-path unaffected (csv.writer пишет все N столбцов
  независимо от header text). Array-style JSON
  (`--header-flatten-style array`) тоже unaffected — у него уже
  shape `[{key, value}, ...]` per-cell. 4 теста в
  `TestDuplicateHeaderDisambiguation` (test_emit_json.py).
- **xlsx2csv не пишет `;`-разделитель для Excel-RU/EU double-click**
  ✅ FIXED 2026-05-12 (TASK 010 §11.7 R26 + same-day vdd-adversarial
  post-merge polish AC-26.6/.7/.8): добавлен флаг
  `--delimiter {,|;|tab|\\t|pipe}` (default `,`). Translation
  делается в `_delimiter_type` argparse-type callable, НЕ в
  post-parse блоке (закрыло coverage-gap, найденный
  /vdd-adversarial). Composable с `--encoding utf-8-sig`. Полный
  Excel-RU recipe: `xlsx2csv.py IN.xlsx --output-dir OUT
  --encoding utf-8-sig --delimiter ';'`. Эквивалент в Python:
  `convert_xlsx_to_csv(IN, OUT, delimiter=';', encoding='utf-8-sig')`
  (kwargs `delimiter` + `encoding` добавлены в `_KWARG_TO_FLAG`
  — R25 «encoding» имел тот же latent bug, fixed retroactively).
  `xlsx2json.py --delimiter ...` теперь emit stderr warning (mirror
  R25 паттерна). Тестов: **12 новых** — внутренний emit-layer (4
  в `test_emit_csv.py`) + CLI-уровень alias resolution (4 в
  `test_cli.py::TestCliDelimiterAliasResolution`) + argparse
...
- **xlsx layout-heavy reports — banner-row vs column-header
  ...
  с merged title-banner в row 1 → банер становится "headers" с
  sticky-fill по merge column-span (после R24 фикса — corectly
  scoped, но всё ещё непригодно как column-name). Workaround:
  `--header-rows 0` (raw rows) либо pre-trim. Возможное
  улучшение — heuristic: if row 1 is a wide merge spanning ≥80%
  of region width AND row 2 has no merges → treat row 1 as
  page-banner (skip), row 2 as column header. Honest-scope
  открыт в TASK 010 §11.7 v2.
- **`enumerate(start=1)` correspondence в `_whole_sheet_region`**
  (discovered 2026-05-12 via `/vdd-multi` Phase-2 logic review):
  scan code предполагает 1:1 между yielded tuples и absolute row
  numbers. Документировано в `_tables.py:342` и `_types.py:200`,
  что openpyxl streaming-mode на некоторых версиях может skip
  entirely-empty rows. Текущий openpyxl 3.1.x consistent, но
...
- **xlsx-10.A** `xlsx_read/` foundation library (M) ✅ **DONE 2026-05-12**
  — **closes code-duplication gap**: общая read-only reader-логика
  (merges, multi-row headers, ListObjects + gap-detection, hyperlinks,
  stale-cache) для xlsx-8 + xlsx-9 (и для xlsx-10.B refactor xlsx-7).
  Шёл **первым** в P0-пакете — теперь xlsx-8/xlsx-9 превращаются
...
- **pdf-12** `pdf_extract.py` + `references/pdf-to-markdown.md` (M) —
  ✅ **DONE (TASK 013, 2026-05-22)**: закрывает частый «сконвертируй
  PDF в md» — reference-дерево решений + dump-хелпер (per-page
  text+tables JSON, scan-детекция через exit 10). НЕ markdown-конвертер
  — композиция остаётся LLM-суждением. pdfplumber.
...
- **pdf-4** `pdf_ocr.py` (M) — hard-blocker: без OCR агент не может
  ...
  `pdf_extract.py` → exit 10 `DocumentScanned` → запустить OCR → повтор.

**P1 — расширение edit-loop'а и data-integrity:**

- **xlsx-10.B** xlsx-7 refactor on `xlsx_read/` (M) — **closes reader-
  duplication window opened by xlsx-10.A**. Чистый refactor (zero
  user-visible value), но: (a) **R2-H2 ownership clock**: 14 дней с
  момента xlsx-9 merge — если упустить, duplication promoted к
  permanent tech debt; (b) **drift-risk** между `xlsx_read/` и
  `xlsx_check_rules/` internal reader растёт со временем (edge-cases
  на merge handling / stale-cache / hyperlink); (c) **no-behavior-
  change gate** жёсткий — 311 unit + 113 E2E + canary saboteur + perf
  budget (100K × 10 rules ≤ 30 s / 500 MB) all unchanged. Рекомендуемое
  окно — **сразу после xlsx-9** (см. Recommended порядок ниже), как
  closing step reader-эпика. Прежняя «day 5+» позиция слишком
  пассивна по отношению к ownership clock'у.
- ~~**docx-6**~~ shipped 2026-05-12 (вместе с docx-6.5/6.6/6.7).
...
- ~~**pdf-7** TOC bookmarks (PDF outline) (S)~~ ✅ DONE 2026-05-22 (TASK 014)
  — weasyprint отдаёт outline из коробки (md2pdf + html2pdf, движок по
  умолчанию, без CSS-флага); `--engine chrome` получил паритет через
  `page.pdf(outline=True, tagged=True)`. Регресс зафиксирован в `test_e2e.sh`.
