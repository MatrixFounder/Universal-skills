...
> **Status (2026-05-11):** ✅ **MERGED**. 9-step atomic chain
> (004.01–004.09) shipped per Task-003 D2 pattern. Body below is the
> historical Analysis-phase spec — preserved verbatim for design
> traceability. See the **Implementation Summary** section appended
> below for post-merge actuals (LOC, test counts, deltas vs spec)
> and links to the round-trip contract spec.
...
## Implementation Summary (post-merge, 2026-05-11)

> Added during `/update-docs` to reflect what actually shipped vs. the
> pre-implementation spec above. The §0–§11 spec is preserved verbatim
> as the design-time contract; this section captures the post-merge
> reality.

### Chain delivery

**9 atomic sub-tasks** shipped within the 8–12 envelope locked in
TASK D5:

| Sub-task | Closes | Status |
|---|---|---|
| 004.01 — skeleton (shim + 7-module package + `.AGENTS.md` placeholder + validate_skill green) | R13.a/b/c, R12.c | ✅ |
| 004.02 — E2E + unit test scaffolding (≥ 11 E2E + ≥ 25 unit + xlsx-8 round-trip fixture + drift-detection) | R11.a-d | ✅ |
| 004.03 — Exceptions module + cross-cutting helpers (cross-5 envelope, cross-7 H1, stdin, post-validate enable-check) | R8, R10.a/d | ✅ |
| 004.04 — `loaders.py` (F1+F2): file/stdin read + 3-shape detection + JSONL parser | R1, R2 | ✅ |
| 004.05 — `coerce.py` (F3): native types + ISO-date coercion + D7 strict-dates rejection | R3, R4, R5 | ✅ |
| 004.06 — `writer.py` (F4): multi-sheet write + styling + sheet-name validation | R5, R6, R7 | ✅ |
| 004.07 — `cli.py` (F5+F7): argparse 8 flags + `_run` linear pipeline + cross-5 envelope catch | R8, R9 | ✅ |
| 004.08 — Post-validate hook logic + synthetic xlsx-8 round-trip + remaining E2E green | R10, R11.b/c | ✅ |
| 004.09 — `references/json-shapes.md` contract freeze + SKILL.md + `.AGENTS.md` + backlog ✅ DONE + 10K perf | R11.e, R12, R13 | ✅ |

### LOC actuals (post-merge)

Package total: **1536 LOC** across `json2xlsx.py` shim (53) +
`json2xlsx/` package (7 modules: `__init__.py` 64, `cli.py` 246,
`cli_helpers.py` 162, `coerce.py` 221, `exceptions.py` 194,
`loaders.py` 320, `writer.py` 276).

Honest overshoot vs original budgets:
- `exceptions.py` 194 vs ≤ 100 (+94) — typed-error details verbosity
  + post-Sarcasmotron `_truncate_utf8_bytes` byte-cap helper.
- `loaders.py` 320 vs ≤ 200 (+120) — error precedence + JSONL parsing
  + multi-sheet validation.
- `writer.py` 276 vs ≤ 220 (+56) — `_union_headers` + sheet-name
  validation + style-copy with drift-detection.
- `cli_helpers.py` 162 vs ≤ 80 (+82) — `run_post_validate`
  subprocess + hermetic-env block.
- `cli.py` 246 vs ≤ 320 — under budget (smaller pipeline than xlsx-7).

All modules within 500-LOC architect-cap.

### Test inventory (post-merge)

- **`tests/test_json2xlsx.py`** — **82 unit tests** across 8 classes
  (TestLoaders 16 + TestCoerce 16 + TestWriter 19 + TestCliHelpers 8
  + TestExceptions 5 + TestRoundTripXlsx8 3 + TestPostValidate 4 +
  TestPublicSurface 2; +9 post-merge additions vs the 73 figure
  originally recorded in `.AGENTS.md`).
- **`tests/test_e2e.sh`** — 14 xlsx-2 cases (3 smoke + 11 inventoried
  tags green): `T-same-path`, `T-happy-single-sheet`,
  `T-happy-multi-sheet`, `T-happy-jsonl`, `T-stdin-dash`,
  `T-invalid-json`, `T-empty-array`, `T-iso-dates`,
  `T-strict-dates-aware-rejected`, `T-envelope-cross5-shape`,
  `T-roundtrip-xlsx8-synthetic`.
- **Round-trip contract**: synthetic golden at
  `tests/golden/json2xlsx_xlsx8_shape.json` (UC-5 lock). Live
  round-trip test `test_live_roundtrip` skipped via
  `@unittest.skipUnless(_xlsx2json_available(), …)` (AQ-5 lock) —
  activates when xlsx-8 lands.
- **Perf**: 10K-row × 6-col budget (≤ 3 s wall) opt-in via
  `RUN_PERF_TESTS=1` (no CI gate; 100K-row JSONL budget DROPPED from
  v1 per honest-scope §11.3 / O4 deferral).

### Round-trip contract artefact

[`skills/xlsx/references/json-shapes.md`](../../skills/xlsx/references/json-shapes.md)
froze at task-004-09. xlsx-8 (`xlsx2json.py`) MUST consume this spec
unchanged when it ships. Live round-trip test wiring lands in xlsx-8's
own commit (xlsx-2 does NOT change).

### Sibling parity established

xlsx-2 became the **pattern reference** for xlsx-3 (`md_tables2xlsx`,
Task-005, MERGED 2026-05-11):

- **Shim + package separation** — Task-003 D2 pattern locked.
- **Public helper signature** `convert_json_to_xlsx(input_path,
  output_path, **kwargs) -> int` routed through `main(argv)` with
  `--flag=value` atomic-token form (VDD-multi M4 protection). xlsx-3
  inherited this 1:1 via ARCH M4 lock.
- **3-way HEADER_FILL drift detection** scaffold — csv2xlsx ↔
  json2xlsx ↔ md_tables2xlsx must all carry byte-identical style
  constants (xlsx-3 test asserts).

### Artefacts archive

- **Code**: `skills/xlsx/scripts/json2xlsx.py` (shim, 53 LOC) +
  `skills/xlsx/scripts/json2xlsx/` package (7 modules, 1483 LOC).
- **Tests**: `tests/test_json2xlsx.py` (82 unit cases) + appended
  `tests/test_e2e.sh` cases.
- **Fixtures**: `examples/json2xlsx_*.json[l]` + synthetic golden
  in `tests/golden/`.
- **Architecture**: archived at
  [`docs/architectures/architecture-003-json2xlsx.md`](../architectures/architecture-003-json2xlsx.md).
- **Plan**: archived at
  [`docs/plans/plan-003-json2xlsx.md`](../plans/plan-003-json2xlsx.md).
- **Reviews**: `docs/reviews/task-004-review.md`,
  `task-004-architecture-review.md`, `task-004-plan-review.md` —
  task-reviewer / architecture-reviewer / plan-reviewer round-1
  records preserved.
- **Per-subtask atomic plans**: `docs/tasks/task-004-01-*.md` through
  `task-004-09-*.md` (9 files).

---

**End of TASK 004 — xlsx-2 — `json2xlsx.py` (✅ MERGED 2026-05-11).**
