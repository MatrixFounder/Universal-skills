...
- **SUBPROCESS-TEXT-DECODE-LOCALE-CLASS** [`subprocess.run(..., text=True)` декодирует вывод ребёнка кодеком локали родителя](issues/subprocess-text-decode-locale-class.md) — severity `SEV-3`, status `open`, opened 2026-09-02
