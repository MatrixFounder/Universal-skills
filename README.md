...
- **[Wiki Ingest Manual](docs/Manuals/wiki-ingest_manual.md)**: Guide to the llm-wiki maintenance meta-skill — vault layout (`_sources/_concepts/_entities`), the four modes (ingest/lint/reindex/query), citation pattern (`[^src-...]` footnotes), contradiction handling, idempotency invariants, the 9 `wiki_ops.py` subcommands, and the fetch → summarize → ingest pipeline.
