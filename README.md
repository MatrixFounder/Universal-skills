...
| **[pptx](skills/pptx/SKILL.md)** | Markdown → PPTX (built-in pptxgenjs renderer with auto-pagination, mermaid diagrams via bundled Cyrillic-capable config, accent stripes — OR `--via-marp` delegation to marp-slide for editorial polish), **`pptx2md.py`** (PPTX → structured Markdown: per-slide sections, GFM tables, sidecar images with sha1-dedup, speaker notes, **opt-in per-image `--ocr`** via system tesseract), **`outline2pptx.js`** (heading-only outline → slide skeleton with TODO placeholders), pptx → PDF, slide thumbnail grids, **`pptx_clean.py`** to drop orphan slides/media after manual editing. | 2 |
