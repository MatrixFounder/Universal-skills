...
`~/.claude/skills/html2md` symlink) picks it up with zero awareness, without polluting the
machine-global environment. Process env wins; opt out via HTML2MD_NO_DOTENV=1; secrets-safe
...
class TestLoadSkillEnv(unittest.TestCase):
    ...
    def test_loads_keys_into_environ(self):
        with tempfile.TemporaryDirectory() as d:
            p = self._env(d, "# comment\nHTML2MD_CHROME_SCROLL=1\nexport HTML2MD_X=\"yes\"\n\n")
            with mock.patch.dict(os.environ, {"HTML2MD_NO_DOTENV": ""}, clear=False):
                os.environ.pop("HTML2MD_CHROME_SCROLL", None)
                os.environ.pop("HTML2MD_X", None)
                cli._load_skill_env(p)
                self.assertEqual(os.environ.get("HTML2MD_CHROME_SCROLL"), "1")
                self.assertEqual(os.environ.get("HTML2MD_X"), "yes")  # quotes + `export ` stripped

    def test_process_env_wins(self):
        with tempfile.TemporaryDirectory() as d:
            p = self._env(d, "HTML2MD_CHROME_SCROLL=1\n")
            with mock.patch.dict(os.environ, {"HTML2MD_NO_DOTENV": "", "HTML2MD_CHROME_SCROLL": "0"}):
                cli._load_skill_env(p)
                self.assertEqual(os.environ["HTML2MD_CHROME_SCROLL"], "0")  # caller's value kept

    def test_opt_out(self):
        with tempfile.TemporaryDirectory() as d:
            p = self._env(d, "HTML2MD_FOO=bar\n")
            with mock.patch.dict(os.environ, {"HTML2MD_NO_DOTENV": "1"}, clear=False):
                os.environ.pop("HTML2MD_FOO", None)
                cli._load_skill_env(p)
                self.assertIsNone(os.environ.get("HTML2MD_FOO"))
    ...
    def test_symlink_skipped(self):
        with tempfile.TemporaryDirectory() as d:
            real = self._env(d, "HTML2MD_FOO=bar\n")
    ...
    def test_inline_comment_and_quotes_stripped(self):
        ...
        self.assertEqual(cli._dotenv_value("~/.html2md/auth-map.json"), "~/.html2md/auth-map.json")
