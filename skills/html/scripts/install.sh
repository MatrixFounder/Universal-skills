#!/usr/bin/env bash
# html skill installer (bead 022-01 baseline; expanded in 022-07).
#
...
if grep -qvE '^\s*(#.*)?$' requirements.txt 2>/dev/null; then
  echo "[html] installing python deps ..."
  ./.venv/bin/python -m pip install -r requirements.txt
fi

if [ "${1:-}" = "--with-chrome" ]; then
  echo "[html] installing Chrome engine (Playwright) ..."
  ./.venv/bin/python -m pip install -r requirements-chrome.txt
  ./.venv/bin/python -m playwright install chromium
fi

if [ -f package.json ]; then
  echo "[html] installing node deps ..."
  npm install --silent
fi
