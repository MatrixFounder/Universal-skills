...
shown. Six fixtures were used, none of which ships with the skill: `slop.md`,
...
(AS-2); and `dont.md`, which names a framework hex inside a `**Don't**` rule
(AS-10). The frontmatter lines that produce each signature are quoted with
...
Steps 1, 2, 4, 5, 6, 7, 8, 9 and 14 compute over the token values themselves,
and on those two routes the values are measurements of an existing product. A
miss there is a finding about that product, not a defect in the file: record
the measurement, the step it misses and the reason in the hand-back, and leave
the value where the measurement put it. Never change a measured value, drop a
measured token or add an unmeasured one to clear a step. Steps 3 and 10 to 13
read the file you wrote and apply on every route.
