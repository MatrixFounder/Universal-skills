...
All outputs quoted below were produced by running the command shown, on
2026-08-28 except the `values` outputs in AS-9, which were produced on
2026-08-29. Seven fixtures were used, none of which ships with the skill:
`slop.md`, written to carry every defect; `derived.md`, written to carry none;
`unrelated.md`, carrying only the AS-1 defect; `dark.md`, carrying only the
AS-11 defect; `loop.md`, whose neutral plane was generated by an even loop
(AS-2); `dont.md`, which names a framework hex inside a `**Don't**` rule
(AS-10); and `dilute.md`, whose `primary` and `error` hold one hex (AS-9). The
frontmatter lines that produce each signature are quoted with the entry, so
every check is reproducible on your own file.
...
def fam(n):
    b = re.sub(r"^(inverse-)?(on-)?(inverse-)?", "", n)
    return "neutral" if re.match(r"(surface|background|outline|scrim|shadow)", b) else re.sub(r"-(container|variant).*$", "", b)
def cmd_values(p):
    src = fm(p); hexes = {}; use = {}
    for n, v, h, s, l, ls, c in colors(p):
        x = v.lower(); hexes[n] = ("#" + "".join(ch * 2 for ch in x[1:]) if len(x) == 4 else x, c)
    for m in re.finditer(r"^[ \t]{2}([\w-]+):[ \t]*\r?\n((?:[ \t]{3,}.*\r?\n)*)", block(src, "components"), re.M):
        for k, v in re.findall(r"([\w-]+):[ \t]*(.+)", m.group(2)):
            for t in re.findall(r"\{colors\.([\w-]+)\}", v):
                use.setdefault(hexes.get(t, ("#??????", 0)), {}).setdefault(t, []).append(f"{m.group(1)}.{k}")
    print(f"{'value':9} {'C':>4} {'n':>3}  token: component.slot")
    for (x, c), toks in sorted(use.items(), key=lambda kv: -kv[0][1]):
        r = {fam(t) for t in toks}
        print(f"{x:9} {c:4.0f} {sum(len(v) for v in toks.values()):3}  "
              + "; ".join(f"{t}: " + ", ".join(v) for t, v in sorted(toks.items()))
              + (f"   !! one value, {len(r)} roles: {', '.join(sorted(r))}" if len(r) > 1 and c >= 6 else ""))
...
Subcommands: `hsl`, `scale`, `lh`, `space`, `refs`, `values`, `dark`.
...
**Detection.** Two ladders. The first command is the AS-1 command and prints
the whole neutral set; the second drops the ink and the inversion tokens from
the selector, leaving the light plane — the surfaces and the hairline.

```bash
python3 /tmp/dm-tokens.py hsl <ABSOLUTE-PATH> '^(?!surface-tint$)(.*(surface|outline|background))'
python3 /tmp/dm-tokens.py hsl <ABSOLUTE-PATH> '^(?!surface-tint$|on-|inverse-)(.*(surface|outline|background))'
```

Plane pass condition — read only the rows the second command prints at
`L* >= 85`; those rows are the plane. Both parts:

- the plane carries at least four distinct `L*` values;
- fewer than half of its non-zero steps lie within 5 percent of their mean.

Whole-ladder pass condition, from the first command:
`max(step) / min(step) >= 2.0` across the neutral ladder, counting only the
non-zero steps.

The two are stated separately because the whole-ladder ratio does not measure
the plane. In every file measured here the largest step of the full ladder is
the drop from the hairline into `outline` and the ink tokens — 28 to 36 L* — so
that ratio reports that the file has a dark text colour. It rejects a ramp
divided evenly across the entire range and nothing finer: a plane generated by
a loop passes it, which is what the plane condition exists to catch.

The plane condition counts steps near their mean instead of dividing the
largest by the smallest because, inside the plane, the ratio does not separate
the cases either. Measured over the plane it is 1.61, 1.30, 3.20, 2.16 and 2.86
for `template-skeleton.md`, `template-product-saas.md`, `template-editorial.md`,
`template-cyrillic.md` and `examples/example-saas-dashboard.md`, and 1.16 for
`loop.md`, whose plane came out of a loop — the two bands nearly touch. The
count within 5 percent of the mean does separate them: 0 of 4, 0 of 9, 0 of 7,
0 of 7 and 0 of 6 for the five files above, 5 of 6 for `loop.md`.
...
Ratio 28.96 / 2.15 = 13.5 on the whole ladder. Its plane is the four rows at
`L* >= 85`, `surface` through `outline-variant`: four distinct values, steps
2.15, 3.11 and 6.69, and none of the three within 5 percent of their mean of
3.98. Passes both conditions.

`loop.md` is the file the whole-ladder ratio cannot see. Its plane command
prints:

```text
L* ladder, lightest first
  background                 #F7F9FB   L*= 97.83 C=  4
  surface                    #F7F9FB   L*= 97.83 C=  4  step=  0.00
  surface-container-low      #F0F4F7   L*= 95.97 C=  7  step=  1.86
  surface-container          #E9EFF3   L*= 94.11 C= 10  step=  1.86
  surface-container-high     #E2EAEF   L*= 92.24 C= 13  step=  1.87
  surface-container-highest  #DBE5EC   L*= 90.39 C= 17  step=  1.85
  surface-variant            #D4E0E8   L*= 88.51 C= 20  step=  1.88
  surface-dim                #CDDAE4   L*= 86.37 C= 23  step=  2.14
  outline-variant            #C6D5E0   L*= 84.47 C= 26  step=  1.90
  outline                    #677B89   L*= 50.52 C= 34  step= 33.95
```

The whole ladder passes: on the AS-1 selector the drop from `outline-variant`
at 84.47 into `outline` at 50.52 is 33.95 and the smallest non-zero step is
1.85, a ratio of 18.4. The plane — the eight rows down to `surface-dim` — fails
the second part: its steps are 1.86, 1.86, 1.87, 1.85, 1.88 and 2.14 against a
mean of 1.91, and five of the six lie within 5 percent of that mean. Those
values were produced by stepping HSL lightness down 2 percent per token at a
fixed hue; `outline`, `on-surface-variant` and `on-surface` were set by hand,
which is what supplies the ratio the whole ladder passes on.
...
On Routes 2 and 3 the first part is a measurement, not a choice:
`references/extraction.md` B.0 puts the number of distinct radius steps in the
**Extracted reliably** column, so a product built on one radius yields one, and
a second radius written to reach the count is invented. Record the single step
and the miss in the hand-back. The second part is prose and applies on every
route.
...
the file does not keep. The four templates in `assets/` each leave radii
unreferenced — `refs` measures three of six for `template-skeleton.md` and
`template-editorial.md`, two of six for `template-product-saas.md`, one of six
for `template-cyrillic.md` — and each of the four names every one of those
tokens in its Shapes section. Read that section against the unreferenced list
token by token: a section that accounts for five of six radii reads as
...
```bash
python3 /tmp/dm-tokens.py values <ABSOLUTE-PATH>
```

`refs` groups by token name, which is the wrong key for this entry. Two names
holding one hex are one color on the screen, and the per-token table clears
both of them. `values` resolves every `{colors.*}` reference to the hex the
token holds, groups the components under that value, and marks a value that
two role families claim.

Pass condition, both parts, stated over distinct hexes rather than names:

- every printed value carries one meaning — one action, one status, one mark;
- no `!!` line.

Neutral names collapse to a single role, so a hex shared by `background` and
`surface`, or by `on-background` and `on-surface`, prints unmarked: MD3 permits
those to coincide, and `template-skeleton.md`, `template-editorial.md` and
`examples/example-saas-dashboard.md` each carry at least one. Values under
`C = 6` are printed and never marked — `#ffffff` under `on-error` and
`on-primary` in `template-skeleton.md`, under `on-primary` and
`surface-container-lowest` in `template-cyrillic.md`, is not a signal.

Measured failure, on a fixture whose `primary` and `error` hold one hex, each
referenced by its own component:

```text
value        C   n  token: component.slot
#c2410c    182   2  error: badge-error.backgroundColor; primary: button-primary.backgroundColor   !! one value, 2 roles: error, primary
```

That file lints at 0 errors and 0 warnings, and `refs` clears it —
`colors.error x1` and `colors.primary x1`, one component and one meaning each.
Grouping by value is what makes the dilution visible. Compare the passing
shape, measured on `assets/template-cyrillic.md`:

```text
#3949ce    149   2  primary: button-primary.backgroundColor, button-ghost.textColor
```

Two slots, one family, one meaning — the primary action.

The single-name case still reads in `refs`. Measured failure:
...
color therefore signals three things and so signals none. Without the
inspector, a count alone is a weaker signal, and it counts one name:
...
count above 2 warrants reading the components; it does not decide them, and it
cannot see a second name holding the same hex at all.

On Routes 2 and 3 how many jobs the accent does is a measurement —
`references/extraction.md` B.0 puts it in the **Extracted reliably** column — so
a `!!` line there is a finding about the product. Record it in the hand-back;
do not split a measured hex in two to clear it.
...
prose. One exemption: a value named inside a `**Don't**` rule, and only there,
is the intended use — a Route 3 extraction that found the framework hex in the
codebase names it in the rule that forbids its return.
...
The second grep is the exemption: it drops the lines that carry a `**Don't**`
marker, and the pipeline's exit status is the second grep's. Keep the marker on
the same line as the value — the filter is per line, so a Don't written as a
bullet under a `### Don't` heading is not exempt and prints.
...
`dont.md` carries both cases: `#2563eb` as `colors.primary`, and `#3b82f6`
inside the rule that forbids it. The unamended grep prints two lines; the
amended pipeline prints one, the violation:

```text
### dont.md, unamended
6:  primary: "#2563eb"
32:**Don't** reintroduce `#3b82f6`. It is the value the marketing pages carried
### dont.md, amended
6:  primary: "#2563eb"
```

With `colors.primary` replaced by a derived value and the Don't left standing,
the amended pipeline exits 1 and the unamended one still reports the Don't:

```text
### dont-clean.md, amended
exit=1
### dont-clean.md, unamended
32:**Don't** reintroduce `#3b82f6`. It is the value the marketing pages carried
exit=0
```
...
Pass condition, in two parts. On every route: no component listing only
`[backgroundColor, textColor]`, and no `!!` line. Where the values are authored
rather than measured — Route 1, and any file written without a source:
`sub-tokens >= 3` for every component. Measured, on a file that mostly passes:
...
**The count is a property of an authored system, not of an extraction.** On
Routes 2 and 3 the third sub-token has to come from the source, and
`references/extraction.md` B.0 grades the six candidates unequally. Only two
sit in the **Extracted reliably** column — the radius system and the number of
hierarchy levels, which yield `rounded` and `typography`. `padding`, `height`,
`width` and `size` in absolute pixels sit in **Extracted approximately**, and
B.0 constrains that column twice: an image does not reliably record its device
pixel ratio, and 15px and 16px are indistinguishable in rendered output. A px
literal is therefore an inference, and it belongs in the INFERRED block of the
B.5 hand-back rather than the MEASURED one — reaching three by adding one
silently is a fabrication with the surface form of a measurement. A control the
capture shows as a bare rule or fill has neither text nor a corner and stops at
two: `examples/example-saas-dashboard.md` records `divider` and
`divider-section` at `sub-tokens=2`, a `backgroundColor` and a `height`,
because the format has no `borderColor` sub-token. Record the two and name the
shortfall in the hand-back; do not invent the third.
...
Two carve-outs are part of the procedure, not exceptions to it.

**A step whose token map is declared in `omitted` does not apply.** Record N/A
against it, the way step 14 reads *if present* — not a pass, and never a reason
to write the map. `colors` carries steps 4 and 5, `typography` steps 2, 6, 7
and 8, `spacing` step 9, `rounded` step 10, `components` steps 11, 12 and 13.

**On Routes 2 and 3 a miss on a measurement step is a property of the source.**
Steps 1, 2, 4, 5, 6, 7, 8, 9, 10, 12, 13 and 14 turn on facts about the
product — the token values themselves, and, in steps 10, 12 and 13, how many
radius steps the product has, how many jobs its accent does, and how much of a
component the source records. `references/extraction.md` B.0 puts the first two
in its **Extracted reliably** column and grades the third no better than
approximate, so none of them is the file's to choose. A miss there is a finding
about that product, not a defect in the file: record the measurement, the step
it misses and the reason in the hand-back, and leave the value where the
measurement put it. Never change a measured value, drop a measured token or add
an unmeasured one to clear a step — a second radius, a split accent and a third
sub-token invented to clear steps 10, 12 and 13 are the fabrications this file
exists to prevent. Steps 3 and 11 read the file you wrote and apply on every
route, as do the two clauses the carve-out does not reach: the stated purpose
behind an unreferenced radius in step 10, and the absence of `!!` lines in
step 13.

1. **No copied defaults.** `grep -inE '#(3b82f6|…|f44336)\b' "$F" | grep -vE "\*\*Don['’]?t\*\*"` — the full
   pattern is in AS-10. Exit status 1. A value named inside a `**Don't**` rule
   is the intended use and is dropped by the second grep.
...
5. **Ramp distribution.** Same output — `max(step)/min(step) >= 2.0` over the non-zero steps. Then the plane selector from AS-2, `'^(?!surface-tint$|on-|inverse-)(.*(surface|outline|background))'` — over its rows at `L* >= 85`, at least four distinct `L*` values and fewer than half the non-zero steps within 5 percent of their mean (AS-2).
...
12. **Accent in one role.** `python3 /tmp/dm-tokens.py values "$F"` — every printed value carries one meaning and no line is marked `!!`; a slot change inside one family is not a finding (AS-9).
13. **Components carry decisions.** The `refs` output again — no component listing only `[backgroundColor, textColor]`, no `!!` lines, and `sub-tokens >= 3` everywhere on an authored file (AS-13).
