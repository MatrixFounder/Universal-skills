...
const preprocess = require('./_html2docx_preprocess');
...
// All preprocessing stages now live in `_html2docx_preprocess.js`. The
// orchestrator runs every stage in the canonical order and returns the
// originalBodyText snapshot that pickContentRoot's body-ratio guard
// needs (captured between chrome strip and reader-mode keyword strip).
// See the preprocess module for stage-by-stage rationale; q-7 extracted
// what used to be ~440 lines of inline procedural transforms here so
// each stage is independently testable from
// `tests/test_html2docx_preprocess.test.js`.
const { originalBodyText: _originalBodyText } = preprocess.preprocessDom($, { readerMode });
