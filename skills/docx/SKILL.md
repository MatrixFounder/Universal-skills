...
- "It's an Obsidian note but it's still Markdown — I'll just run it through `md2docx.js`." → **WRONG**. `md2docx.js` exits **0** and writes a **valid** `.docx` that is missing content. Measured on a real vault note: the YAML frontmatter is stripped unconditionally (so `participants`, which live only there, vanish), `![[embed]]` and `[[wikilink]]` reach `marked` as `text` tokens and land in the document as literal `![[…]]` strings, and a bare space in an image path also lexes as text — every vault path has spaces. Exit 0 guarantees nothing here. Use `md2docx.js --obsidian` (or `obsidian2md.js` first).
- "The note has no `![[embeds]]`, so plain `md2docx.js` is fine." → **WRONG** if it has frontmatter. D1 fires on every note: the frontmatter is dropped silently and metadata that lives nowhere else goes with it.
...
- "The doc has `$x^2$` in it, `md2docx.js` will just write it as text and that's fine." → **WRONG before TASK 031.** `$…$`/`$$…$$` used to reach the document as literal characters, exit 0, no warning — the same silent-loss class as the Obsidian defects above. As of TASK 031, math is **on by default**: it converts to a native OOXML `<m:oMath>` object, editable in Word like any equation authored with the equation editor. Only `--no-math` restores the old literal-text behaviour.
...
- **Render inline `$…$` and display `$$…$$` math as native OOXML `<m:oMath>` objects** via `md2docx.js` — on by default, editable in Word like an equation authored with its own equation editor, not a raster image. Pipeline: KaTeX renders TeX to MathML in-process, `mathml2omml` converts to OMML, spliced into the paragraph as a fresh object per occurrence. A standalone `$$…$$` alone between blank lines (or the sole content of a table cell) gets a dedicated centred paragraph; a 4-column table whose 1st/3rd cells are blank — the Pandoc `|  | $$…$$ |  | (N) |` equation-with-number convention — gets asymmetric column widths so the formula isn't clipped to 1/4 of the page width. A formula that fails to render degrades to its literal source text plus a stderr warning (never a silent drop); `--strict-math` turns that into a hard exit 1 instead. `--no-math` restores pre-TASK-031 literal-text behaviour exactly. Honest scope: no Word-native automatic equation numbering (a source document's own numbers, sitting outside the `$$…$$` delimiters, convert unchanged); math inside `**bold**`/`*italic*`/a link still renders as a real object but without that styling; math in an image's alt text renders as literal text (the `ImageRun.altText` field has nowhere to splice a run into).
- **Convert an Obsidian vault note to `.docx` in one command** via `md2docx.js --obsidian [--vault-root DIR]`, or to clean CommonMark via `obsidian2md.js` (both share `_obsidian_lib.js`). Closes four measured silent-loss defects: YAML frontmatter dropped, `![[embed]]` and `[[wikilink]]` unrecognised, and spaces in image paths. **Frontmatter is re-emitted as a two-column table by default** (`--frontmatter table|render|strip`) — a GFM table, so it goes through the same `table` token path as every other table and carries the identical borders and header shading. Attachments resolve in Obsidian's own order (`.obsidian/app.json` → note directory → literal path → NFC-folded vault-wide index); callouts (`> [!warning] Title`), `==highlight==`, `%%comment%%`, `#inline-tags`, and `- [ ]` task boxes are normalised; fenced/indented/inline code is masked and passes through byte-identical, so a note documenting Obsidian syntax is not rewritten. Opt-in `--transclude` inlines `![[Note]]` with cycle protection (depth ≤ 3, visited-set) and confines the target to the vault — inlining text from outside it would be an arbitrary-file read driven by a note somebody else wrote. Missing attachment → placeholder + stderr warning (exit 0), or exit 8 `AssetNotFound` under `--strict-assets`. Honest scope: math (`$…$`) still reaches the document as text (backlog `docx-11`); a `|300` width hint is an upper bound and never upscales, unlike Obsidian itself; `.webp` is named as a file rather than embedded, because the docx image layer cannot carry it; and an indented code block *nested inside a list* is treated as list content, so Obsidian syntax inside one is still rewritten.
...
- **Commands**:
  - `node scripts/md2docx.js INPUT.md OUTPUT.docx [--header "TEXT"] [--footer "TEXT"] [--page-size A4|Letter] [--landscape] [--margins T,R,B,L] [--no-math] [--strict-math]` — math (`$…$`/`$$…$$` → native `<m:oMath>`) is **on by default**; `--no-math` disables it entirely (literal text, pre-TASK-031 behaviour); `--strict-math` exits 1 on the first formula KaTeX cannot render instead of the default degrade-to-literal-text-plus-warning.
  - `node scripts/md2docx.js INPUT.md OUTPUT.docx --obsidian [--vault-root DIR] [--frontmatter table|render|strip] [--lang ru|en|auto] [--links text|italic] [--inline-tags strip|keep] [--transclude] [--strict-assets]` — the one-command Obsidian route. `--vault-root` defaults to the nearest ancestor of the note containing `.obsidian/`, else the note's own directory. Every Obsidian flag **requires** `--obsidian`; using one without it exits **1** with `--vault-root requires --obsidian` (exit 1 is `md2docx.js`'s code for every usage error, unchanged since TASK 019). Flag order is irrelevant. `--strict-assets` exits **8**, in plain text — `md2docx.js` has no `--json-errors`.
  - `node scripts/obsidian2md.js INPUT.md OUTPUT.md [--vault-root DIR] [--frontmatter table|render|strip] [--lang ru|en|auto] [--links text|italic] [--inline-tags strip|keep] [--transclude] [--strict-assets] [--json-errors]` — Obsidian-flavoured Markdown → CommonMark, no OOXML involved. Defaults: `--frontmatter table`, `--lang auto` (reads the note's `lang:` key, else `en`), `--links text`, `--inline-tags strip`. Exit codes: 0 success, 1 I/O, 2 invalid argument / usage, 6 same-path self-overwrite refused (`SelfOverwriteRefused`), 8 attachment not found under `--strict-assets` (`AssetNotFound`). `--json-errors` emits the standard `{v:1, error, code, type, details}` envelope on every non-zero path. Output is idempotent: a second run over the same input is byte-identical. A CommonMark document with no Obsidian syntax passes through byte-identical.
...
- **Read scope, `--obsidian` / `obsidian2md.js` only** (containment is compared on `realpath`, not on path strings — a directory symlink shipped inside a vault walks straight through a prefix check): resolving an Obsidian note additionally **reads** (never writes) files the command line did not name — attachments under the vault root, `<vault>/.obsidian/app.json`, and, under `--transclude`, sibling notes. The vault root is `--vault-root`, else the nearest ancestor of the note containing `.obsidian/`, else the note's directory. `.obsidian/`, `.git/`, `node_modules/` and `.trash/` are never walked, directory symlinks are never followed, and the index is built once per run. `attachmentFolderPath` is untrusted input that travels with a vault: an absolute value, or one resolving outside the vault, is refused with a warning. A note may still name an absolute path in its own link — the same capability plain Markdown `![](/abs/path.png)` already has, and it is not confined.
...
- **Local verification**:
  ...
  - `node scripts/md2docx.js examples/fixture-obsidian-vault/note.md /tmp/obs.docx --obsidian --vault-root examples/fixture-obsidian-vault && unzip -l /tmp/obs.docx | grep -cE 'word/media/.*\.(png|jpg)'` — the committed mini-vault note: exit 0, **5** embedded images, frontmatter inside a `<w:tbl>`. Full acceptance set (A1-A16) in `scripts/tests/test_obsidian2md.py`, 90 cases.
  - `node scripts/md2docx.js examples/fixture-math.md /tmp/math.docx && unzip -p /tmp/math.docx word/document.xml | grep -o '<m:oMath' | wc -l` — exit 0, ≥4 native math objects. `test_md2docx_math.py` (26 cases).
...
| **Obsidian note → .docx** | `node scripts/md2docx.js "<vault>/note.md" out.docx --obsidian --page-size A4` |
| Obsidian note → clean .md | `node scripts/obsidian2md.js "<vault>/note.md" clean.md` |
| Markdown with `$…$`/`$$…$$` → .docx (math on by default) | `node scripts/md2docx.js input.md output.docx` |
| Markdown → .docx, math disabled | `node scripts/md2docx.js input.md output.docx --no-math` |
...
- [scripts/obsidian2md.js](scripts/obsidian2md.js) — Obsidian note → CommonMark CLI (TASK 030). Thin argv/exit-code wrapper; all rules live in `_obsidian_lib.js`.
- [scripts/_obsidian_lib.js](scripts/_obsidian_lib.js) — the conversion core, required in-process by both `obsidian2md.js` and `md2docx.js --obsidian`. Knows nothing about OOXML, so the rules are testable as text-in/text-out and stay reusable by peer skills.
- [examples/fixture-obsidian-vault/](examples/fixture-obsidian-vault/) — committed mini-vault driving `scripts/tests/test_obsidian2md.py`: `app.json`, embeds with size hints, a `%`-in-name attachment, a space-named directory, and a mutual-transclusion pair.
- [scripts/_math_lib.js](scripts/_math_lib.js) — `$…$`/`$$…$$` extraction, sentinel substitution, and KaTeX → MathML → `mathml2omml` OMML batch rendering (TASK 031). Required in-process by `md2docx.js`; knows nothing about `docx`-js or OOXML beyond the OMML string it returns.
- [examples/fixture-math.md](examples/fixture-math.md) — committed fixture driving `scripts/tests/test_md2docx_math.py`: inline math, a standalone display formula, a Pandoc-style numbered equation table, math inside `**bold**`, currency/escaped-dollar negative cases, a formula inside a fenced code block, and one intentionally-malformed formula.
