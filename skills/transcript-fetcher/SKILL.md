...
- `scripts/_human.py` — the presentation channel's opposite contract: prose **obeys** the caller's codec instead of overriding it. `say()` (a `print` drop-in) and `HumanArgumentParser` (`--help`/usage) degrade `—✓✗→⚠…` to ASCII spellings per character only when the stream cannot carry them, so UTF-8 output is byte-identical; anything the table cannot spell falls back to `backslashreplace` rather than raising — which is also what makes a lone surrogate from a `surrogateescape` filename survivable, and it keeps a dead reader from rewriting the exit status to 120.
