...
- **Single URL**:
  ...
  ```
  Optional flags: `--lang ru` (default), `--prefer manual|auto` (default `manual`), `--with-description`, `--description-only`, `--cookies-file PATH`, `--json-errors`, `--debug` (stage logging to stderr), `--asr-allow-cloud` (opt-in cloud ASR), `--asr-model <id>`, `--asr-timeout-sec N`, `--max-duration-min N` (X: transcribe only the first N minutes — clips the download), `--auth-map PATH` (per-host cookies), `--cookies-from-browser BROWSER` (X: load cookies from a local browser via yt-dlp).
