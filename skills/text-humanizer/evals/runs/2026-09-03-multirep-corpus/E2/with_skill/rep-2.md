...
Note that `robust_mode=true` disables the ceiling. While robust mode is on, the client retries
until the deadline passes.

Buffers must align to a 64-byte boundary. A misaligned buffer raises `EINVAL`.

Workers are added when the queue depth exceeds 10000 messages.