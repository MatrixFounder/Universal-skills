...
`robust_mode=true` disables the ceiling. While robust mode is on, the client retries until
the deadline passes.

Buffers must align to a 64-byte boundary. A misaligned buffer raises `EINVAL`.

The scheduler adds workers when the queue depth exceeds 10000 messages.