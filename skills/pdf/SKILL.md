...
- [scripts/pdf_extract.py](scripts/pdf_extract.py) — dumps a PDF's per-page text + tables to structured JSON via `pdfplumber`, with scan detection (image-only document → exit `10`) and opt-in artwork extraction (`--extract-images DIR`: rasters copied as stored + vector figures cropped via Poppler). A dump, not a Markdown converter.
