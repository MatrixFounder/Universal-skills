...
- **Render HTML / web archives to PDF** via `html2pdf.py` — same weasyprint pipeline, natively handles `.html`/`.htm`, `.mhtml`/`.mht` (MIME HTML, "Save as Single File"), and `.webarchive` (Apple WebKit/Safari archive). Bundled stylesheet on by default; `--no-default-css` for fully-styled inputs (BI dashboards, branded reports); `--css EXTRA.css` stacks on top. `--reader-mode` strips navigation/ads and renders only the main article (Safari Reader View parity, Confluence-priority candidate list with body-ratio guard for `<main>` and longest-match per selector). Validated across **Fern, Mintlify, GitBook, Confluence, Хабр, vc.ru** and generic blogs — 34/34 fixtures pass in both modes. Universal preprocessing applied unconditionally to both modes: external `<link rel=stylesheet>` blocks are stripped (bypasses pathological site CSS — Хабр-style content drops, vc.ru-style layout hangs); offline URL fetcher refuses `http(s)://` fetches with no timeout (no more multi-minute network stalls); SIGALRM watchdog caps render at `--timeout 180` (override via `$HTML2PDF_TIMEOUT`, 0 disables); ad networks (`adfox`/`taboola`/`tm-banner`/etc.) and interactive chrome (`<button>` unwrapped, `<video>`/`<audio>`/`<iframe>` dropped) cleaned site-agnostically; small decorative SVG icons (≤64 px width/height, `aria-hidden="true"`, FontAwesome `prefix=fa*`, Tailwind `h-4 w-4`) stripped, large content diagrams kept; empty `<a href="#anchor">` wrappers (left over after icon strip) removed; **table-based code blocks flattened** — Fern/Mintlify/Docusaurus syntax-highlighting tables (`<pre><table><tr class="code-block-line">`) collapse to plain `<pre><code>` to avoid weasyprint's pagination bug where mid-table rows interleave with following paragraphs (manifested as cut-off code blocks at line 15+). Plus draw.io / Confluence inline SVG diagrams render correctly (foreignObject labels → SVG `<text>`, viewBox synthesised for clip-free scaling); CSS `light-dark()` resolved to light variant; web-font `@font-face` blocks stripped (prevents garbled glyphs from CDN-subsetted fonts). Code blocks, blockquotes, and inline `<code>` get markdown-preview-style typography (light grey background, rounded border, monospace, `white-space: pre-wrap` for long-line wrapping inside the page width).
