...
Measured before the fix: `wiki_ops.py --help` returned rc 1 and 0 bytes where UTF-8 returned 2940.
...
from wiki_ingest import _stdout as H  # noqa: E402
...
class TestTheMachineChannelIgnoresTheLocale(unittest.TestCase):
    ...
    Everything above obeys the caller's codec. `write_json` must NOT: JSON is
...
class TestTheRealCommands(unittest.TestCase):
    ...
    COMMANDS = {
        "wiki_ops.py --help": ["skills/wiki-ingest/scripts/wiki_ops.py", "--help"],
        "wiki_ops.py scan --help": ["skills/wiki-ingest/scripts/wiki_ops.py",
                                    "scan", "--help"],
    }
